...
// 将 MatrixXd 转换为 double*，返回指向新分配内存的指针
double* matrixXdToDoublePtr(const Eigen::MatrixXd& mat) {
    int rows = mat.rows();
    int cols = mat.cols();
...
// 将 VectorXd 转换为 double*，返回指向新分配内存的指针
double* vectorXdToDoublePtr(const Eigen::VectorXd& vec) {
    int size = vec.size();
...
MatrixXd doublePtrToMatrixXd1(const double* ptr, int rows, int cols) {
    ...
    for (int j = 0; j < cols; ++j) {
        for (int i = 0; i < rows; ++i) {
            mat(i, j) = ptr[i + j * rows];
        }
    }
    return mat;
}

// 将 double* 转换为 VectorXd
